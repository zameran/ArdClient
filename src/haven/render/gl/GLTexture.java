...
public abstract class GLTexture extends GLObject implements BGL.ID {
	...
	static int magfilter(Sampler smp) {
		switch (smp.magfilter) {
			...
			case LINEAR:
				return (GL.GL_LINEAR);
			default:
				throw (new IllegalArgumentException(String.format("%s.magfilter: %s", smp, smp.magfilter)));
		}
	}
	...
	static int minfilter(Sampler smp) {
		if (smp.mipfilter == null) {
			switch (smp.minfilter) {
				...
				case LINEAR:
					return (GL.GL_LINEAR);
				default:
					throw (new IllegalArgumentException(String.format("%s.minfilter: %s", smp, smp.minfilter)));
			}
		}
		switch (smp.mipfilter) {
			...
			default:
				throw (new IllegalArgumentException(String.format("%s.mipfilter: %s", smp, smp.mipfilter)));
		}
	}
	...
	static int wrapmode(Texture.Wrapping w) {
		switch (w) {
			...
			case CLAMP_BORDER:
				return (GL3.GL_CLAMP_TO_BORDER);
			default:
				throw (new IllegalArgumentException(String.format("wrapmode: %s", w)));
		}
	}

...
public class MapView extends PView implements DTarget, Console.Directory {
	...
	public class FreeCam extends Camera {
		private float dist = Utils.getpreff("badcamdistdefault", 50.0f);    //private float dist = 50.0f;
		public float distwheel = Utils.getpreff("badcamdistwheeldefault", 5.0f);
		private float distminimal = Utils.getpreff("badcamdistminimaldefault", 5.0f);
		private float elev = Utils.getpreff("badcamelevdefault", (float) Math.PI / 4.0f);     //private float elev = (float) Math.PI / 4.0f;
		private float angl = Utils.getpreff("badcamangldefault", 0.0f);     //private float angl = 0.0f;
		private Coord dragorig = null;
		...
		public void tick(double dt) {
			Coord3f cc = getcc();
			cc.y = -cc.y;
			distwheel = Utils.getpreff("badcamdistwheeldefault", 5.0f);
				...
				public Pipe group(int idx) {
					switch (idx) {
						...
						case idx_idp:
							return (idp);
						default:
							return (back.group(idx - idx_back));
					}
				}
	...
	*/
	...
	public boolean mousedown(Coord c, int button) {
		...
		if (button == 2) {
			...
			new Click(c, ui.modflags(), button).run();
		}
		return (true);
	}
	...
	public boolean drop(final Coord cc, Coord ul) {
		new Hittest(cc, ui.modflags()) {
			public void hit(Coord pc, Coord2d mc, ClickData inf) {
				if ((configuration.nodropping || configuration.nodropping_all) && !ui.modctrl) {
					// no dropping at all or when we are on water
					boolean nodropping = false;
					if (configuration.nodropping_all) {

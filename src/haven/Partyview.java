...
public class Partyview extends Widget {
	...
	private void update() {
		if (party.memb != om) {
			...
			List<Entry<Member, Avaview>> wl = new ArrayList<Entry<Member, Avaview>>(avs.entrySet());
			Collections.sort(wl, new Comparator<Entry<Member, Avaview>>() {
			...
			int i = 0;
			for (Entry<Member, Avaview> e : wl) {
				e.getValue().c = new Coord((i % 2) * 43, (i / 2) * 43 + 24);
				i++;
			}
		}
		for (Entry<Member, Avaview> e : avs.entrySet()) {
